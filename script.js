...
function createNewGeneration(){
    let copyState = checkboxes.slice().map(item => item.map(item => item.checked));
    for(let i = 0; i < height; i++){
        for(let j = 0; j < width; j++){
            let neighbors = countLiveNeighbors(i, j, copyState);
            if(copyState[i][j] && (neighbors > 3 || neighbors < 2)) checkboxes[i][j].checked = false;
